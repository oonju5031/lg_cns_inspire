...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        log.debug(">>> JwtAuthenticationFilter User EndPoint: {}", request.getRequestURI());
...
//            response.setHeader("Access-Control-Allow-Headers", "true");
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            log.debug(">>> JwtAuthenticationFilter Not Authorized.");
            chain.doFilter(request, response);
            return;
        }
        ...
        log.debug(">> JwtAuthenticationFilter Validation Check");
        ...
        try {
            ...
            log.debug(">>> JwtAuthenticationFilter claims get email: {}", email);

            // JwtProvider에 의해 Role이 입력된 경우에만 해당
            String role = claims.get("role", String.class);
            log.debug(">>> JwtAuthenticationFilter claims get role: {}", role);
